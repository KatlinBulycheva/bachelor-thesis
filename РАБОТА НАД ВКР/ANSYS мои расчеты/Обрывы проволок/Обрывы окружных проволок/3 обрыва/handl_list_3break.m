...
Wp = pi*d^3/16;    % мм^3   момент сопротивления на кручение
sig_t = 400;       % МПа
...
el_bott_41 = 759; % Последний элемент в последнем регулярном блоке
el_last    = 2957; % Последний элемент в файле

fid = fopen('3breaks_listing.lis', 'rt');
...
n_result = [1.6863 1.2520 0.8809];
figure(1)
fig = bar(n_result, 0.5, 'FaceColor',[0 .6 .6]); grid on;
xlabel('количество обрывов', 'FontSize', 18)
ylabel('запас прочности {\bf n}', 'FontSize', 18)
%annotation('line',[.900,.13],[.570,.570],'LineStyle','--')
